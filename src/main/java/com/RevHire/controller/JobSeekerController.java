...
public ResponseEntity<?> getProfile(@PathVariable Long userId) {
    ...
    Optional<Resume> resumeOpt =
            resumeRepo.findTopBySeekerSeekerIdOrderByResumeIdDesc(profile.getSeekerId());
    ...
    if (resumeOpt.isPresent()) {
        ...
        response.put("jobTitle", resume.getObjective());
        ...
        if (!educationList.isEmpty()) {
            response.put("education",
                    educationList.get(0).getDegree() + " - " +
                            educationList.get(0).getInstitution());
        }
        ...
        if (!skillList.isEmpty()) {
            ...
            String skills = skillList.stream()
        ...
        List<ResumeCertification> certList =
                certificationRepo.findByResumeResumeId(resume.getResumeId());

        if (!certList.isEmpty()) {

            String certs = certList.stream()
    ...
    int completion = calculateProfileCompletion(profile, response);

    response.put("profileCompletion", completion);
...
/*
    ...
    public ResponseEntity<?> updateProfile(
        ...
        if(data.get("totalExperience")!=null)
            profile.setTotalExperience(Integer.parseInt(data.get("totalExperience").toString()));
...
/*
    ...
    private int calculateProfileCompletion(
        ...
        if(response.get("education")!=null)
            filled++;

        if(response.get("skills")!=null)
            filled++;

        if(response.get("experience")!=null)
            filled++;

        if(response.get("certifications")!=null)
            filled++;
